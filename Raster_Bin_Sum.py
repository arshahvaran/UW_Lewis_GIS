...
clip_dir = r"C:\Users\PHYS3009\Desktop\Plastics_NetCDF\Layer2_Sum_Subbasins"
watersheds_dir = r"C:\Users\PHYS3009\Desktop\Plastics_NetCDF\Vectors"
csv_dir = r"C:\Users\PHYS3009\Desktop\Plastics_NetCDF\Layer2_Sum_Subbasins"

watersheds = ["greatlakes_subbasins_erie.shp", "greatlakes_subbasins_huron.shp",
              "greatlakes_subbasins_ontario.shp", "greatlakes_subbasins_michigan.shp",
              "greatlakes_subbasins_superior.shp"]
...
for bin_num in range(1, 7):
    ...
    if rasters_to_sum:  # Ensure list is not empty
        ...
        for watershed in watersheds:
            watershed_name = watershed.split('_')[2].split('.')[0]
            clipped_raster_name = f"l2_b{bin_num}_na_s_{watershed_name}.tif"
...
df_final['Watershed'] = df_final['FileName'].apply(lambda x: x.split('_')[-1].split('.')[0])  # Extracting watershed name
...
df_sum_dict = {watershed: '{:.4e}'.format(value) for watershed, value in df_sum.items()}
df_sum_df = pd.DataFrame(list(df_sum_dict.items()), columns=["Watershed", "? Per Days"])
print(df_sum_df)


# Save dataframes to csv files named
df_final.to_csv(os.path.join(csv_dir, "table1.csv"), index=False)
df_sum_df.to_csv(os.path.join(csv_dir, "table2.csv"), index=False)
